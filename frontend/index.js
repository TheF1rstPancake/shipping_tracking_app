...
function ShippingBlock() {
  ...
  const carrierField = activeTable.getFieldByIdIfExists(
    globalConfig.get("carrierFieldId")
  );
  ...
  const selectedRecord = useRecordById(
    ...
    {
      fields: [trackingField, carrierField],
    }
  );
    ...
    <Box padding={3}>
      <Box>
        {!showCreateShipmentScreen &&
          cursor.activeTableId === orderTableId &&
          selectedRecordId &&
          trackingNumber && (
            <TrackingDetails
              ...
              carrierName={selectedRecord.getCellValueAsString(carrierField)}
            />
          )}
        {!showCreateShipmentScreen &&
          cursor.activeTableId === orderTableId &&
        ...
        {cursor.activeTableId != orderTableId && (
          <Heading>
            Shipping block is only working on table <i>{orderTable.name}</i>
          </Heading>
        )}
        {!selectedRecordId && cursor.activeTableId === orderTableId && (
          <h1>Select an order in your table to get started.</h1>
        )}
      </Box>
    </Box>
  );
