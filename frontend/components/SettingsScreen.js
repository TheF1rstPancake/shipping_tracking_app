...
const SettingsScreen = ({ setVisible }) => {
  ...
  const [getShippoAPIKey, setShippoAPIKey] = useState(
    globalConfig.get("shippoAPIKey")
  );
  const [fromAddress, setFromAddress] = useState(
    globalConfig.get("fromAddress")
  );
  const [getOrdersTableId, setOrdersTableId] = useState(
    globalConfig.get("ordersTableId")
  );
  const [selectedTableId, setSelectedTableId] = React.useState(null);
  const [selectedStatusFieldId, setSelectedStatusFieldId] = React.useState(
    null
  );
  const [
    selectedTrackingNumberFieldId,
    setSelectedTrackingNumberFieldId,
  ] = React.useState(null);
  ...
  return (
    <Box padding={3}>
      ...
      <FormField label="In which table do you track orders?">
        <TablePickerSynced table={table} globalConfigKey="orderTableId" />
      </FormField>
      ...
      <FormField label="Which column do you use to track order status?">
      ...
      <FormField label="Which column will have the shipping provider/carrier?">
        <FieldPickerSynced table={table} globalConfigKey="carrierFieldId" />
      </FormField>
